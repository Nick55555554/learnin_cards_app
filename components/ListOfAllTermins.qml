...
Item {
    ...
    SequentialAnimation {
            ...
            // И делаем новую карточку видимой
            PropertyAnimation {
                target: terminLoader.item
                property: "opacity"
                to: 1
    ...
    onUserIdChanged: loadTermins(userId)
    ...
    function loadTermins(userId) {
        terminsModel.clear();
        var termins = databaseManager.getTerminsByUserId(userId);
